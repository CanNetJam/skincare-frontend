        ...
        <>
            {videos[0]!==undefined ?
                <>
                    {videos.map((a)=> {
                        return (
                            <div key={a} className="p-0 sm:h-[300px] sm:w-[200px] h-[200px] w-[125px] rounded-md overflow-hidden relative shadow-md shadow-gray-700">
                                <div className='sm:block hidden drop-shadow-[0_5px_5px_rgba(0,0,0,1)] absolute z-50 top-0 sm:h-8 sm:w-8 h-6 w-6 sm:m-4 m-2 right-0 sm:text-4xl text-3xl justify-center items-center cursor-pointer' onClick={()=> {
                                ...
                                <video
                                    onClick={handleVideo}
                                    className="bg-blue-300 h-full w-full object-cover"
                                    loop
                                    autoPlay={true}
                                    ref={vidRef}
...
/*
                            <div key={a} className="z-50 sm:h-[350px] sm:w-[200px] h-[200px] w-[125px] rounded-md shadow-md overflow-hidden relative bg-blue-300">
                                <ReactPlayer className='z-10' volume={1} loop={true} muted={true} playing={true} height={'90%'} width={'100%'} controls={true} url={`https://res.cloudinary.com/${import.meta.env.VITE_CLOUDNAME}/video/upload/f_auto,q_60/${a}.mp4`}/>
                            </div>

                            <div key={a} className="sm:h-[350px] sm:w-[250px] h-[200px] w-[125px] rounded-md overflow-hidden relative">
                                <div className=' drop-shadow-[0_5px_5px_rgba(0,0,0,1)] absolute z-50 top-0 sm:h-8 sm:w-8 h-6 w-6 sm:m-4 m-2 right-0 sm:text-4xl text-3xl grid justify-center items-center cursor-pointer' onClick={()=> {
                                    setFloatingVideo(false)
                                    setStop(true)
                                }}>
                                    <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path fill='white' d="m12 10.93 5.719-5.72c.146-.146.339-.219.531-.219.404 0 .75.324.75.749 0 .193-.073.385-.219.532l-5.72 5.719 5.719 5.719c.147.147.22.339.22.531 0 .427-.349.75-.75.75-.192 0-.385-.073-.531-.219l-5.719-5.719-5.719 5.719c-.146.146-.339.219-.531.219-.401 0-.75-.323-.75-.75 0-.192.073-.384.22-.531l5.719-5.719-5.72-5.719c-.146-.147-.219-.339-.219-.532 0-.425.346-.749.75-.749.192 0 .385.073.531.219z"/></svg>
